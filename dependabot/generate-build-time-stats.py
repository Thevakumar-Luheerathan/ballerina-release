...
ballerina_org_name="Thevakumar-Luheerathan"
        ...
        if wait_cycles < MAX_WAIT_CYCLES:
            ...
            if not pending:
                ...
                if passing:
                    ...
                    print("[Info] Automated build-time-stats generation PR PR has failed checks." + "\n" +\
                        "Please visit <" + pr.html_url + "|the build page> for more information")
...
def approve_pr(module, auto_merge_pull_requests, pr_number):
    if (auto_merge_pull_requests.lower() == 'true') & module['auto_merge']:
        ...
        repo = r_github.get_repo(ballerina_org_name + '/' + module['name'])
        pr = repo.get_pull(pr_number)
        try:
            pr.create_review(event='APPROVE')
            print(
                "[Info] Automated build time stats generation PR approved for module '" + module['name'] + "'. PR: " + pr.html_url)
        except Exception as e:
            raise e
